...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $existingSlug = $db->fetchOne("SELECT id FROM projects WHERE slug = ?", [$slug]);
    if ($existingSlug) {
    ...
    if (empty($errors)) {
        ...
        if (empty($errors)) {
            $sql = "INSERT INTO projects (title, slug, description, content, featured_image, category, tags, project_url, client, date_completed, status, display_order, created_at, updated_at) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())";
            
            if ($db->query($sql, [$title, $slug, $description, $content, $featured_image, $category, $tags, $project_url, $client, $date_completed, $status, $display_order])) {
                $success = true;
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="date" class="form-control" id="date_completed" name="date_completed"
                                               value="<?php echo htmlspecialchars($_POST['date_completed'] ?? ''); ?>">
                                    </div>
